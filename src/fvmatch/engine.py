...
from fvmatch.model.live import LiveState, conditional_scoreline_matrix
...
def analyze_live_match(
    ...
    *,
    ...
    """Run the fair-value pipeline for an in-play fixture.

    Same as :func:`analyze_match`, but step 3 uses
    :func:`~fvmatch.model.live.conditional_scoreline_matrix` with ``state``.
    """
    cfg = config or default_settings
    eh = elo_home if elo_home is not None else get_rating(home)
    ea = elo_away if elo_away is not None else get_rating(away)
    ...
    matrix = conditional_scoreline_matrix(
        ...
        intensity_profile=cfg.live_intensity_profile,
    )
    p_home, p_draw, p_away = marginal_hda(matrix)
    model_ps = {"home": p_home, "draw": p_draw, "away": p_away}
